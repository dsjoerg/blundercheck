...
# the per-game DFs
game_df = material_df.join(eloscored_df, how='outer')
...
# the per-playergame DFs
pg_dfs = [depthstats_df, elo_df, crunched_df, move_aggs, wmove_aggs]
pg_df = concat(pg_dfs, axis=1)
winners_df = pg_df[pg_df[' YOU WERE HERE.
IF YOU ARE GOING TO DO IT BY WINNER/LOSER THEN YOU NEED TO DECIDE WHAT TO DO WITH DRAWN GAMES.
