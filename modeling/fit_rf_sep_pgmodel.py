...
excluded_features = ['elo', 'opponent_elo', 'elo_advantage', 'elo_avg', 'winner_elo_advantage', 'ols_error', 'gamenum', 'rfr_prediction', 'rfr_error', 'index', 'ols_avg_error', 'ols_ea_error', 'ols_wea_error']
...
if do_breadth_cv:
    ...
    for train_index, test_index in kf:
            shell()
            msg("fit ae")
            foo_ae = rfr.fit(train.iloc[train_index][features], train.iloc[train_index]['elo_avg'], sample_weight=train.iloc[train_index]['weight'].values)
            msg("pred ae")
            ae_preds = rfr.predict(train.iloc[train_index][features])
            in_ae_mae = mean_absolute_error(ae_preds, train.iloc[train_index]['elo_avg'])
            msg("pred out ae")
            out_ae_preds = rfr.predict(train.iloc[test_index][features])
            out_ae_mae = mean_absolute_error(out_preds, train.iloc[test_index]['elo_avg'])

            msg("fit ea")
            foo_ea = rfr.fit(train.iloc[train_index][features], train.iloc[train_index]['elo_advantage'], sample_weight=train.iloc[train_index]['weight'].values)
            msg("pred ea")
            ea_preds = rfr.predict(train.iloc[train_index][features])
            in_ea_mae = mean_absolute_error(ae_preds, train.iloc[train_index]['elo_advantage'])
            msg("pred out ea")
            out_ea_preds = rfr.predict(train.iloc[test_index][features])
            out_ea_mae = mean_absolute_error(out_preds, train.iloc[test_index]['elo_advantage'])

            msg("combining ae and ea")
            combo_out_pred = out_ae_preds + out_ea_preds
            combo_out_mae = mean_absolute_error(combo_out_pred, train.iloc[test_index]['elo'])
            
            msg("reconciling combinations")
            shell()
